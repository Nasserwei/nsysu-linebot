...
def build_course_carousel() -> TemplateMessage:
    ...
    columns = [
        CarouselColumn(
            thumbnail_image_url="https://mis.nsysu.edu.tw/images/logo.png",
        ...
        CarouselColumn(
            ...
            actions=[
                ...
                URIAction(
                    label="查看課程地圖",
                    uri="https://mis.nsysu.edu.tw/course-map",
                ),
            ],
        ),
        CarouselColumn(
...
_COURSE_DATA = {
    "required": {
        ...
        "link": "https://mis.nsysu.edu.tw/curriculum",
    },
    "elective": {
        "title": "📝 選修學分規定",
        "items": [
    ...
    "double_major": {
        ...
        "link": "https://aa.nsysu.edu.tw/double-major",
    },
}
...
def build_quick_reply_menu() -> QuickReply:
    ...
    return QuickReply(
        items=[
            QuickReplyItem(action=MessageAction(label="📚 選課資訊", text="選課資訊")),
            QuickReplyItem(action=MessageAction(label="📢 最新公告", text="最新公告")),
            QuickReplyItem(action=MessageAction(label="🐒 校園生態", text="台灣獼猴怎麼辦")),
            QuickReplyItem(action=MessageAction(label="🍖 燒肉推薦", text="附近哪裡有燒肉")),
            QuickReplyItem(action=MessageAction(label="🌶️ 麻辣推薦", text="麻辣火鍋推薦")),
            QuickReplyItem(action=MessageAction(label="💻 GitHub學生", text="如何申請 GitHub Copilot")),
            QuickReplyItem(action=MessageAction(label="🌆 高雄旅遊", text="高雄好玩的地方")),
            QuickReplyItem(action=MessageAction(label="🏠 宿舍資訊", text="宿舍怎麼申請")),
        ]
    )
